   ...
   <div class="card recent-sales overflow-auto">
     <div class="card-body">
       <div class="d-flex justify-content-between align-items-center mb-3">
         ...
         <div>
           <button class="btn btn-sm btn-primary me-2" data-bs-toggle="modal" data-bs-target="#modalImportMatkul">
             <i class="bi bi-upload"></i> Import Mata Kuliah
           </button>
     ...
     <div class="modal-content">
       <form id="formTambahMatkul">
         ...
         <div class="modal-body">
           <input type="hidden" name="fakultas_id" id="hiddenFakultasMatkul">
           <div class="mb-3">
             <label class="form-label">Fakultas</label>
             <select class="form-select" id="fakultasMatkul" disabled required>
           ...
           <div class="mb-3">
             ...
             <select class="form-select" name="prodi_id" id="prodiMatkul" required>
               ...
               <option value="{{ $prodi->id }}" data-fakultas="{{ $prodi->fakultas_id }}">{{ $prodi->nama_prodi }}</option>
               @endforeach
             </select>
           </div>
           <div class="mb-3">
     ...
     <div class="modal-content">
       <form id="formEditMatkul">
         ...
         <div class="modal-body">
           ...
           <input type="hidden" name="fakultas_id" id="hiddenEditFakultasMatkul">
           <div class="mb-3">
             <label class="form-label">Fakultas</label>
             <select class="form-select" id="editFakultasMatkul" disabled required>
               <option selected disabled>Pilih Fakultas</option>
               @foreach($fakultas as $fak)
               <option value="{{ $fak->id }}">{{ $fak->nama_fakultas }}</option>
               @endforeach
 ...
 <div class="modal fade" id="modalImportMatkul" tabindex="-1" aria-labelledby="modalImportMatkulLabel" aria-hidden="true">
   <div class="modal-dialog">
     <div class="modal-content">
       <form id="formImportMatkul" enctype="multipart/form-data">
         @csrf
         <div class="modal-header">
           <h5 class="modal-title">Import Data Mata Kuliah dari Excel</h5>
           <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Tutup"></button>
         </div>
         <div class="modal-body">
           <div class="mb-3">
             ...
             <div class="form-text">
               ...
               Header yang diperlukan: nama_fakultas, nama_prodi, nama_mata_kuliah, kode_matakuliah<br>
               Header opsional: sks, keterangan
             </div>
           </div>
         </div>
         <div class="modal-footer">
 ...
 <script>
   document.addEventListener('DOMContentLoaded', function() {
     ...
     // Form Import Mata Kuliah
     document.getElementById('formImportMatkul').addEventListener('submit', function(e) {
       e.preventDefault();
       var formData = new FormData(this);

       fetch("{{ route('mata-kuliah.import') }}", {
           method: 'POST',
           body: formData,
           headers: {
         ...
         .then(data => {
           if (data.success) {
             ...
             Swal.fire('Gagal!', data.error || 'Terjadi kesalahan saat mengimpor data.', 'error');
           }
         })
     ...
     document.getElementById('prodiMatkul').addEventListener('change', function() {
       var selectedOption = this.options[this.selectedIndex];
       var fakultasId = selectedOption.getAttribute('data-fakultas');
       if (fakultasId) {
         document.getElementById('fakultasMatkul').value = fakultasId;
         document.getElementById('hiddenFakultasMatkul').value = fakultasId;
       }
     });

     // Auto-select Fakultas when Prodi changes in Edit Modal
     document.getElementById('editProdiMatkul').addEventListener('change', function() {
       var selectedOption = this.options[this.selectedIndex];
       var fakultasId = selectedOption.getAttribute('data-fakultas');
       if (fakultasId) {
         document.getElementById('editFakultasMatkul').value = fakultasId;
         document.getElementById('hiddenEditFakultasMatkul').value = fakultasId;
       }
     });
